...
The code will save all the following files
  1. "ansatz_1_seed_4.pt" : Stores the network (torch nn.Module  inherited class)
  2. "config_4.json" : Stores the configuration used for training
  3. "rewards_per_episode_seed_4.npy" : Stores a numpy array containing the rewards of the agent per episode 
  4. "train_rewards_summary_seed_4.jpg" : Plot showing the rewards per episode 
## Extracting the compression ratio:
...
Once the correct path is provided, the plots will be generated on their own. Note that this accesses the files *"rewards_per_episode_seed_*.npy"*.
